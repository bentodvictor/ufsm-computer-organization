...
#        Percorre a lista e armazena o campo código dos nodos em um array (loop)
...
loop:    # Laço que percorre todos nodos da lista
    beq $t1, $zero, end
    lw $t2, 0($t1)          # t2 <- t1->n (LinkedList.first->n)
    sw $t2, 0($t7)          # array[t7] <- Node.n
    addi $t7, $t7, 4        # t7++
    lw $t1, 4($t1)          # t1 <- t1->next (t1 aponta o próximo nodo da lista)
    j loop
end:
