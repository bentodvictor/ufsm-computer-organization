...
.text
    # O novo nodo esta alocado no endereco 0x100100130
    li $t3, 0x10010030      # t3 <- 0x10010030

    la $t0, LinkedList      # t0 <- &LinkedList
    lw $t1, 4($t0)          # t1 <- LinkedList.tail (t1 aponta o �ltimo nodo da lista)
    
    # Faz o �ltimo nodo da lista apontar o novo nodo
    sw $t3, 4($t1)          # t1->next <- t3 (LinkedList.tail->next aponta o novo nodo)
    
    # Seta o novo nodo como o �ltimo da lista
    sw $t3, 4($t0)          # LinkedList.tail aponta o novo nodo
    sw $zero, 4($t3)        # O �ltimo nodo aponta para NULL
